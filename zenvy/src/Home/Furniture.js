...
function Furniture() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      // Direct fetch from Furniture collection
      const res = await fetch(`${API_URL}/api/unified/furniture`);
      const data = await res.json();
      console.log("Furniture products:", data);
  ...
  return (
    <div className="container py-4">
      ...
      <h2 className="text-center mb-4">Furniture & Decor</h2>
      <div className="row g-4">
        {products.length === 0 ? (
          <div className="text-center py-5">
            <p>No products found in Furniture & Decor category.</p>
          </div>
        ) : (
