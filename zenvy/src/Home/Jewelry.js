...
function Jewelry() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/api/unified/category/Jewelry`);
      const data = await res.json();
      console.log("Jewelry products:", data);
  ...
  return (
    <div className="container py-4">
      ...
      <h2 className="text-center mb-4">Jewelry</h2>
      <div className="row g-4">
        {products.length === 0 ? (
          <div className="text-center py-5">
            <p>No products found in Jewelry category.</p>
          </div>
        ) : (
