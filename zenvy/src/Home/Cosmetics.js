...
function Cosmetics() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      // Direct fetch from HealthBeauty collection
      const res = await fetch(`${API_URL}/api/unified/health-beauty`);
      const data = await res.json();
      console.log("Health & Beauty products:", data);
  ...
  return (
    <div className="container py-4">
      ...
      <h2 className="text-center mb-4">Health & Beauty</h2>
      <div className="row g-4">
        {products.length === 0 ? (
          <div className="text-center py-5">
            <p>No products found in Health & Beauty category.</p>
          </div>
        ) : (
